...
| `condition_record_id` / `set_result.frozen_condition_record_id` | Set, with the frozen market-invalidation record at matched-cycle persistence | One identity for the Part IV §12 record, bound to decision_cycle_id, set_result_id, symbol, Set/version/family/direction, original snapshot and configuration identity/version/content digest. frozen_condition_record_id points to this same condition_record_id, not a second record. Retain record content/digest; replay never remints or rebuilds from current configuration. |
| Frozen `condition_id` | Set configuration producer; copied unchanged into the matched frozen condition record | Identifies the configured typed predicate, threshold/reference, horizon, freshness, unavailable policy and evidence selector inside that record. The record/condition pair is retained across activation and replay; no independently reinterpreted predicate or threshold drift. |
| F-013 `evidence_digest` | Set, when the selected factual evidence for a condition evaluation is retained | Binds the actual selected evidence identity/content, frozen record/condition and cycle/result/linked active entry remainder. For TRUE invalidation, invalidated_at is that evidence's authoritative effective time, not evaluation/receipt time. No invented digest or TRUE evidence for the separate unavailable branch. |
| Order Cancel `signal_id` | Set; INVALIDATION at the first logical TRUE condition/evidence/remainder signal; MONITORING_UNAVAILABLE is the already-acquired unavailable_requirement_id, not a second ID | Cause and exact cycle/result/tranche/symbol/accepted-entry binding are immutable. INVALIDATION preserves condition_record_id, condition_id, evidence_digest and the original evidence-effective invalidated_at. MONITORING_UNAVAILABLE preserves the original unavailable_at, unavailable_reason_code and optional record presence/value, with signal_id equal to unavailable_requirement_id. Identical evaluation/delivery/restart returns the committed identity/content. No symbol/time/latest-result targeting; no filled-exposure close authority. See Order Cancel Signal Semantics. |
| F-013 `unavailable_requirement_id` | Set, atomically acquiring-or-joining the first durable MONITORING_UNAVAILABLE requirement for an exactly bound original accepted entry, before publication | Unique semantic key is (F013_MONITORING_UNAVAILABLE, decision_cycle_id, set_result_id, tranche_id, original entry client_order_link_id), inside existing account/environment lineage. Retain the original Order Placed/client/native entry pair; do not use partial-fill quantity, lifecycle revision, condition observation, retry or market-recovery time as a new key. One sticky requirement per entry coalesces all unavailable conditions. Opaque encoding remains implementation-local; the persisted key-to-ID mapping is unique and replay-stable. Signal ID is this same ID. Same key returns original content; changed content or a second ID for a known key is an integrity/reconciliation condition. |
| F-013 unavailable effective-time / reason binding | Set, on the first durable MONITORING_UNAVAILABLE transition establishing the requirement | Persist unavailable_at as that state transition's effective timestamp, plus its transition/source identity and order; never invent a missing-market-evidence or TRUE-invalidation time. After reducer UNAVAILABLE, initial reason order is ACTIVATION_IDENTITY_INVALID, FROZEN_RECORD_INVALID_OR_UNRESOLVED, INVALID_CONDITION, REQUIRED_EVIDENCE_UNAVAILABLE; retain all known failures locally. Freeze primary reason/time and optional condition_record_id absence/value. Deferred target resolution preserves the original incident/time and uses persisted transition order, never recovery-arrival order. If target lineage is unresolved, retain the integrity/reconciliation condition under the existing exact source/event identity and send no guessed-target envelope; only authoritative linkage to that original accepted entry permits acquire-or-join. |
| F-013 unavailable delivery / terminal binding | Set persists requirement, content and outbox atomically; Lifecycle owns durable receipt, authoritative reconciliation, cancel execution and terminal/tombstone state | Receipt is a conditional reconcile-first request, not TRUE evidence. Restore immutable requirement/content, publication/delivery progress, Lifecycle receipt/deduplication, cancel/reconciliation progress and current remainder revision/tombstone. Signal replay is idempotent. Only positive active-remainder proof admits cancellation; terminal proof makes the request no longer applicable and returns on existing Order Placed/entry-lifecycle events. Retain original key/identity and terminal deduplication; market recovery, partial fill, restart or delayed placement cannot erase/remint/reactivate it. Missing/conflicting recovery state is not no requirement. No new owner/edge or Portfolio release. |
