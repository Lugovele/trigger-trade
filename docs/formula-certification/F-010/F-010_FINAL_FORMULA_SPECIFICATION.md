...
| Approved candidate | `F-010_REVISED_SPEC_CYCLE_2.md` |
...
This final specification preserves the Cycle 2 revised candidate semantics
...
F010-C03: permitted type, pool, capability predicate and reason-precedence definitions
```
...
### 5.6 Capability, directional pool, basic pool and traversable pool
...
`reference_geometry.levels` collection projected into this specification as
`references.levels[]`.
...
The `basic_pool` is the subset of `directional_pool` that also passes Section
5.4 strict favorable-side price geometry for the active direction.
...
| `references.levels[]` | Market Handoff v4 `reference_geometry.levels` | Frozen governed levels available as-of `matched_at`; collection presence, schema validity, handoff binding and replayability determine geometry capability. |
...
references.levels[] present, schema-valid, identity-bound and replayable from the frozen handoff
...
### Geometry capability unavailable
...
directional_pool = []
result = NO_FAVORABLE_SIDE_GEOMETRY
```

The same result applies when the persisted handoff lacks a replayable
schema-valid `reference_geometry.levels` collection, after higher-precedence
identity and missing-primitive checks.

### Geometry capability available but no basic candidates
...
t = 0.25
references.levels[] =
  L1: level_type = SWING_HIGH_15M, price = 100.25, available_at <= matched_at

directional_pool = [L1]
basic_pool = []
result = NO_ELIGIBLE_REFERENCE
```

`L1` is a governed directionally permitted level, so capability is available;
it fails the strict LONG favorable-side test because `price > E + t` is false.

### Geometry capability available with selectable target
...
t = 0.25
references.levels[] =
  L2: level_type = SWING_HIGH_15M, price = 104, available_at <= matched_at

directional_pool = [L2]
basic_pool = [L2]
rounded_target_price = 104
result = AVAILABLE
```

Restart with the same frozen handoff and F-008 entry must reproduce the same
primary reason or selected target for all three geometry-capability fixtures.
...
| Adversarial Strategy Reviewer | APPROVE_WITH_LIMITATIONS | Binding validation, capability separation and terminal traversal rules withstand the reviewed edge cases; defensive branches are distinguished. |
...
limitations_and_research: Preserve Sections 22-24 and the limitations recorded in the Full Council review.
...
| 2 | Replaced the undefined geometry-capability phrase with a deterministic predicate over frozen Market Handoff v4 `reference_geometry.levels`. | Closes remaining F010-C03 capability blocker without adding a new dependency. |
| 2 | Added fixtures for unavailable capability, available capability with no basic candidates and available capability with a selectable target. | Verifies primary reason separation and restart equality. |
