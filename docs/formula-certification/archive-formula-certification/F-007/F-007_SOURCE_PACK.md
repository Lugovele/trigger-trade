...
|---|---|---|
| Formula ID | F-007 | `docs/FORMULA_METRICS_CATALOG.md` |
| Formula name | Position Rules SHORT formula | `docs/FORMULA_METRICS_CATALOG.md` |
...
| Catalog canonical-definition pointer | `docs/trading-methodology/methodology/POSITION_RULES.md` :: §38 Mathematical Formula - SHORT | `docs/FORMULA_METRICS_CATALOG.md` |
| Catalog note | Includes entry, stop candidate, ATR-risk checks, and tick rounding for SHORT. | `docs/FORMULA_METRICS_CATALOG.md` |
...
|---:|---|---|---|---|
| 1 | `docs/FORMULA_METRICS_CATALOG.md` | F-007 row | CATALOG / GATE | Identifies F-007, owner, scope, canonical pointer and downstream impact. |
...
| 4 | `docs/trading-methodology/methodology/POSITION_RULES.md` | Part III §§14-21 | REFERENCE SELECTION | Defines SHORT default reference priority by Set family, ranking within priority type, audit trail and corroborating-level diagnostics. |
...
| 7 | `docs/trading-methodology/methodology/POSITION_RULES.md` | §38 Mathematical Formula - SHORT | FORMULA SUMMARY | Provides the compact SHORT formula with `R`, `A`, `E`, raw/adjusted/rounded stop and ATR risk checks. |
| 8 | `docs/formula-certification/F-003/F-003_FINAL_FORMULA_SPECIFICATION.md` | Final spec | CERTIFIED_FORMULA | Supplies certified 15m ATR semantics consumed through Market Handoff. |
| 9 | `docs/formula-certification/F-005/F-005_FINAL_FORMULA_SPECIFICATION.md` | Final spec | CERTIFIED_FORMULA | Supplies certified Set direction classifier and Market Handoff/no-handoff boundary. |
| 10 | `docs/formula-certification/F-008/F-008_FINAL_FORMULA_SPECIFICATION.md` | Final spec | CERTIFIED_FORMULA | Supplies certified planned entry reference and limit order price consumed by F-007. |
| 11 | `docs/formula-certification/F-006/F-006_FINAL_FORMULA_SPECIFICATION.md` | Final spec | CERTIFIED_FORMULA | Supplies certified LONG-side sibling precedent for Dynamic SL boundary handling; does not define SHORT arithmetic. |

## 3. Purpose and Trading Context

F-007 is the Position Rules-owned SHORT Dynamic Stop Loss calculation. It
converts:

```text
frozen Set Market Handoff
+
certified SHORT direction
+
...
```text
usable Dynamic SL candidate for a SHORT Position Plan
```
...
F-007 protects the SHORT thesis by anchoring the stop beyond governed adverse
market structure above the entry, adding ATR noise buffer, enforcing minimum
technical distance, rejecting stops that are too wide, and rounding outward to
exchange tick size. It does not determine Set direction, choose planned entry,
determine take profit, compute size/notional, compute final risk/reward, approve
Portfolio capital, or submit/cancel orders.
...
`NONE` is invalid for SL creation. `LONG` belongs to F-006.
...
For the SHORT branch, only governed adverse-side highs can become eligible
under the geometry rules below.

### Candidate eligibility recomputed vs planned entry

The Set's cached side is audit-only for Dynamic SL selection. F-007 recomputes
...
### SHORT default reference priorities
...
highest-priority eligible reference is selected, F-007 calculates and validates
...
```text
rounded_stop = ceil(adjusted_stop / t) * t
```
...
```text
post_rounding_risk_price = rounded_stop - E
...
|---|---|---|
| `direction` | F-005 / Market Handoff | Must be committed `SHORT`; `NONE` invalid and `LONG` belongs to F-006. |
...
F-007 produces a `dynamic_sl` candidate with:
...
| Post-rounding percent diagnostic | Percent or percentage-style diagnostic as defined by the output contract; exact source formula not specified in the extracted F-007 source text |
...
F-007 is SHORT-only. Its stop lies above the planned entry and above the
selected adverse reference after outward rounding. A higher stop increases
SHORT loss distance, so ceiling rounding is outward for SHORT. F-007 does not
infer SHORT from score or price movement; the SHORT direction must already be
established by F-005 and present in the immutable Market Handoff.
...
LONG/SHORT and Part V tick-size alignment. The extracted F-007 source includes
SHORT stop candidate construction and SHORT tick rounding; F-009 remains a
downstream/broader stop-boundary formula and is not certified by this Source
Pack.

## 13. Precision

The source requires decimal-safe arithmetic. F-007 consumes exact canonical
...
```text
rounded_stop = ceil(adjusted_stop / tick_size) * tick_size
```

The Source Pack did not locate a separate F-007-specific quotient rounding rule
...
F-007 is deterministic from the frozen Market Handoff, F-008 planned entry,
...
| F-006 | CERTIFIED_FORMULA | Supplies certified LONG Dynamic SL sibling precedent for shared boundary conventions; not used for SHORT arithmetic. |
...
No numeric worked example was found in the extracted F-007 source text.

## 21. Explicit Source Gaps

1. The catalog says F-007 includes "entry, stop candidate, ATR-risk checks, and tick rounding for SHORT," while F-008 already certifies planned entry selection. The likely boundary is that F-007 consumes F-008 entry rather than recalculating it, but Council should verify this boundary.
2. F-009 separately covers stop calculation and stop rounding. The F-007 source itself includes SHORT stop construction and rounding. Council should verify whether F-007 may certify the SHORT branch candidate while F-009 later certifies cross-side stop/rounding integration.
3. The output contract includes a post-rounding risk-distance `pct` field, but the extracted F-007 source text does not specify its exact formula.
...
5. F-006 Council review required positive rounded stops and deterministic Unicode code-point ID ordering for the LONG sibling. The extracted F-007 source contains the same likely underspecified patterns; Council should determine whether the same closures are required for SHORT.

## 22. Reviewer Handoff Summary

F-007 appears to be a complete SHORT Dynamic Stop Loss candidate:

```text
R = selected adverse SHORT reference
A = received ATR_15m
...
pre_rounding_risk_atr = (adjusted_stop - E) / A
...
post_rounding_risk_atr = (rounded_stop - E) / A
if post_rounding_risk_atr > 2.00:
    reject SL_TOO_WIDE
```

The Council should review specification correctness, determinism, dependency
compatibility, ownership boundaries and trading fitness for the SHORT stop-loss
role. Special attention should be paid to F-007/F-008 and F-007/F-009
boundaries, whether F-006's approved positive-stop and deterministic tie-break
closures should be mirrored, exact handling of `pct` diagnostics, and whether
the extracted research parameters and no-fallback/no-retry semantics are
complete enough for certification.
