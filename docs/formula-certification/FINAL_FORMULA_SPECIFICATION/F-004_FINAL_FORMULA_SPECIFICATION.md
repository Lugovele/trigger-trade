...
# F-004 — Set Normalization, Percentile and Score Calculation

**Artifact:** `F-004_FINAL_FORMULA_SPECIFICATION.md`
...
This final specification incorporates the Council-defined arithmetic subset
from Cycle 1, the Cycle 2 closure of the local 5m ATR dependency, and the
Cycle 3-approved population-selector fixes required for F004-C02:
...
This document is the self-contained final specification approved by the Full
Council. It does not modify active methodology files and does not implement
runtime logic.
...
All eight expert perspectives approved the Cycle 2 candidate with limitations.
No role identified a remaining blocker requiring another specification cycle.

| Perspective | Final verdict | Assessment |
|---|---|---|
...
| Market Regime & Context Analyst | APPROVE_WITH_LIMITATIONS | Metric-specific first full days and restricted initialization exclusions define reproducible warmup populations. Historical percentiles provide context; thresholds and comparability across regimes remain empirical. |
| Quant Strategy Researcher | APPROVE_WITH_LIMITATIONS | Population variance, prescribed square-root rounding, inclusive ranks, clipping and exact weighted aggregation retain the accepted arithmetic. Population membership has explicit boundaries and minimums. |
| Risk & Trade Management Architect | APPROVE_WITH_LIMITATIONS | Missing required evidence remains unavailable, including local-momentum veto inputs. A calculable five-factor score cannot override required-data failure or authorize downstream eligibility. |
| Execution & Exchange Mechanics Specialist | APPROVE_WITH_LIMITATIONS | Separate 5m ATR ancestry, work precision, source identities and restart requirements remain intact. The midnight selector is reproducible; analytical approval confers no execution authority. |
| Adversarial Strategy Reviewer | APPROVE_WITH_LIMITATIONS | Partial inception days, initialization prefixes, later invalid observations, midnight self-inclusion, insufficient references and duplicate identities have defined outcomes. No remaining specification counterexample requiring revision was identified. |
| Performance & Strategy Diagnostics Analyst | APPROVE_WITH_LIMITATIONS | Population identities, availability reasons, gates, side-specific vetoes and contributions support attribution. Calibration, factor redundancy and incremental performance after costs remain research questions. |
...
F004-C04: CLOSED
F004-C05: CLOSED
F004-C06: RETAINED_EMPIRICAL_QUESTION_NON_BLOCKING
F004-C07: RETAINED_NON_BLOCKING_LIMITATION
